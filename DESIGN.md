---
name: PubDates
description: A row of drawn glasses; tap one and your mates hear it; each glass empties in 15 minutes, the row stays until you have seen it, then two hours.
colors:
  pint-amber: "#E68A00"
...
PubDates is a native iOS consumer app played straight at the craft level of Instagram, Pinterest and Tikkie: system backgrounds, system type, system navigation, one committed accent. What owns the screen is a horizontal row of nine drawn glasses under the large title. Every glass rests empty (the label under it names the drink); tapping one fills it to the brim, opens the camera, and the new row springs in at the top of the feed with the same glass full. Over the next 15 minutes that glass drains, on the feed row and on the map pin; the row stays until you have seen it and two hours more, and it is the only row that person has: a new drink replaces the old one. The feed is who is drinking right now, not a timeline to scroll; the camera is the last cell beside the row, not a competing control.
...
- **Stein** (`pint-amber`): the house pils in a straight mug, the one glass whose decoration is a handle beside the body rather than a stem under it.
...
- **Label** (`.caption`): captions under the nine glasses and the camera cell (regular, secondary, one line, 0.7 minimum scale, Amber Ink on the favourite); semibold in status pills; semibold, uppercase, 0.6 pt tracking in the Mates eyebrows.
...
- **Reaction picker**: a `.medium` sheet from a mate's row — six preset emoji in a three-wide grid of 60 pt wash tiles, then one field that takes any emoji or a few words, then Send. One reaction per mate per drink, so it closes on send.
- **Caption strip** (camera review): a flat black band at 50% across the full width of the photo, no radius, no blur, no margin — Snapchat's band, not a system material. White `.title3` semibold, centred, 12 pt vertical and 20 pt side padding, up to three lines and 80 characters. It starts at 62% down the photo, drags vertically within it, and rides above the keyboard while you type. Empty means no band. The band is drawn into the uploaded JPEG at the same fractions, so what you typed on is what your mates see.
...
- **Level:** 0 is empty, 1 is the brim. Unlogged glasses rest at 0 (Tim: "all glasses should be empty unless filled"). A logged drink starts at 1 and drains linearly to 0 over 15 minutes; the row and the pin stay until you have seen them, then two hours more.
...
Nine glasses in canonical order — the beers first, small to dark (pils, pint, stein, special beer, stout), then wine, bubbles, cocktail, whisky — one tap each, in a horizontal `ScrollView` with view-aligned paging.
